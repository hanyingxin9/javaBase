...
public           ok           ok              ok                ok
    ...
    构造器（构造方法 ， constructor）  ------>  Constructor.java
        ...
        理解问题的三个角度 : 3W （what? why? how?）

        关键字 : this的使用   ---------->  ThisKeyWord.java
         1. this可以用来修饰 : 属性 方法 构造器
         2. this修饰属性和方法时理解成 当前对象 或者 当前正在创建的对象（构造器中使用this）


         this修饰 调用属性和方法 :

         在类的方法中，我们通常使用 "this.属性" "this.方法"的方式调用当前对象的属性或者方法
         但是通常情况下我们省略 "this." 特殊情况下 ，如果方法的形参和类的属性同名时 ， 我们必须显式
         的使用 "this.变量" 的方式，表明此变量是属性而非形参

         在类的构造器中，我们通常使用 "this.属性" "this.方法"的方式调用当前正在创建的对象的属性或者方法
         但是通常情况下我们省略 "this." 特殊情况下 ，如果构造器的形参和类的属性同名时 ， 我们必须显式
         的使用 "this.变量" 的方式，表明此变量是属性而非形参
         ...
         1. 我们在类的构造器中 ， 可以显式的使用 "this(形参列表)"方式 ， 调用本类中指定的其他构造器
           （每个构造器中都做一些重复的操作，比如初始化，就会造成冗余。在一个构造器中写好，其他构造器直接调用该构造器）
         2. 构造器不能通过 "this(形参列表)"方式调用自己
         3. 如果一个类中有 n个构造器 ，则最多有 n-1 构造器中使用了 "this(形参列表)" (空参构造器什么也不调)
         4. 规定 : "this(形参列表)"必须声明在当前构造器的首行 ， 所以有 5
         ...
         package关键字的使用 :
            ...
            1. 为了更好的管理项目中的类 提供包的概念
            2. 使用package声明类或接口的包 声明在源文件首行
            3. 包 属于标识符 遵循标识符的命名规则,规范（xxxyyyzzz） "见名知意"
            4. 每 "."一次 , 就代表一层文件目录
            5. 同一个包下 , 不能命名同名的接口或类
               不同的包下 , 可以命名同名的接口或类（可以通过全包名区分）


         jdk中主要的包介绍 :

           1. java.lang  包含一些java语言的核心类 如 String Math Integer System Thread 提供常用功能
           2. java.net   包含执行与网络相关的操作的类和接口
           3. java.io    包含能提供多种输入/输出功能的类
           4. java.util  包含实用工具类 如定义系统特性 接口的集合框架类 使用与日期日历相关的函数
           5. java.text  包含java格式化相关的类
           6. java.sql   包含java进行JDBC数据库编程的相关类/接口
           7. java.awt   包含构成抽象窗口工具集（abstract window toolkits）的多个类，这些类被用来构建和管理
                         应用程序的图形化用户界面（GUI） B/S  C/S
